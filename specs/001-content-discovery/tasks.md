...
- [ ] T001 Adicionar dependências `react-router-dom` e `lucide-react` em `package.json`
- [ ] T002 [P] Criar template de variáveis de ambiente com configurações do TMDB em `.env.example`
- [ ] T003 [P] Configurar estilos globais, reset e variáveis do tema escuro (dark streaming) em `src/index.css`
...
- [ ] T004 Implementar cliente base da API TMDB com tratamento de URLs e erros em `src/services/tmdb.js`
- [ ] T005 [P] Implementar componente de botão reutilizável e suas variantes estilizadas em `src/components/Button/Button.jsx` e `src/components/Button/Button.css`
- [ ] T006 [P] Implementar componente de cabeçalho global com logo e navegação em `src/components/Header/Header.jsx` e `src/components/Header/Header.css`
- [ ] T007 Implementar componente de layout principal com Header e Outlet do React Router em `src/layouts/MainLayout/MainLayout.jsx` e `src/layouts/MainLayout/MainLayout.css`
- [ ] T008 Configurar roteamento declarativo da aplicação com as rotas `/`, `/descobrir`, `/resultados` e `/filme/:id` em `src/routes/AppRoutes.jsx`
- [ ] T009 Atualizar componente raiz para renderizar o provedor de rotas da aplicação em `src/App.jsx`
...
- [ ] T010 [P] [US1] Implementar componente de barra de progresso do quiz em `src/components/ProgressBar/ProgressBar.jsx` e `src/components/ProgressBar/ProgressBar.css`
- [ ] T011 [P] [US1] Implementar card interativo de seleção de opção com ícones em `src/components/OptionCard/OptionCard.jsx` e `src/components/OptionCard/OptionCard.css`
- [ ] T012 [P] [US1] Implementar componente de apresentação da pergunta e opções do quiz em `src/components/QuizQuestion/QuizQuestion.jsx` e `src/components/QuizQuestion/QuizQuestion.css`
- [ ] T013 [P] [US1] Implementar componente de estado de carregamento com spinner estilizado em `src/components/Loading/Loading.jsx` e `src/components/Loading/Loading.css`
- [ ] T014 [P] [US1] Implementar componente de exibição de erro com botão de retry em `src/components/ErrorMessage/ErrorMessage.jsx` e `src/components/ErrorMessage/ErrorMessage.css`
- [ ] T015 [P] [US1] Implementar componente de estado vazio para buscas sem resultados em `src/components/EmptyState/EmptyState.jsx` e `src/components/EmptyState/EmptyState.css`
- [ ] T016 [P] [US1] Implementar card de exibição de filme/série com pôster, título, ano e avaliação em `src/components/MovieCard/MovieCard.jsx` e `src/components/MovieCard/MovieCard.css`
- [ ] T017 [US1] Implementar grid responsivo para renderização da lista de MovieCards em `src/components/MovieGrid/MovieGrid.jsx` e `src/components/MovieGrid/MovieGrid.css`
- [ ] T018 [US1] Implementar funções de consulta de gêneros e descoberta de filmes/séries (`getRecommendations`, `getGenres`) em `src/services/tmdb.js`
- [ ] T019 [US1] Implementar página Home com banner de destaque e CTA "Encontrar algo para assistir" em `src/pages/Home/Home.jsx` e `src/pages/Home/Home.css`
- [ ] T020 [US1] Implementar página Discover com controle de etapas do quiz (1 a 3) e estado de preferências em `src/pages/Discover/Discover.jsx` e `src/pages/Discover/Discover.css`
- [ ] T021 [US1] Implementar página Results consumindo a API TMDB com tratamento de estados (loading, success, empty, error) em `src/pages/Results/Results.jsx` e `src/pages/Results/Results.css`
...
- [ ] T022 [US2] Implementar função de busca de detalhes por ID de filme ou série (`getContentDetails`) em `src/services/tmdb.js`
- [ ] T023 [US2] Implementar página Details com imagem de destaque, sinopse, metadados e navegação de retorno em `src/pages/Details/Details.jsx` e `src/pages/Details/Details.css`
- [ ] T024 [US2] Integrar ação de clique no MovieCard para navegar até a rota de detalhes `/filme/:id` em `src/components/MovieCard/MovieCard.jsx`
...
- [ ] T025 [US3] Integrar ação "Refazer Quiz" no cabeçalho da página de resultados e no EmptyState em `src/pages/Results/Results.jsx`
- [ ] T026 [US3] Integrar ação "Nova Descoberta" na barra de ações da página de detalhes em `src/pages/Details/Details.jsx`
...
- [ ] T027 [P] Revisar responsividade fluida para resoluções mobile (360px a 430px) e desktop em `src/index.css`
- [ ] T028 [P] Remover arquivos não utilizados do boilerplate padrão do Vite (`src/App.css`, ícones SVG não utilizados) em `src/`
- [ ] T029 Executar verificação de linter e corrigir avisos/erros de código com `npm run lint` em `eslint.config.js`
